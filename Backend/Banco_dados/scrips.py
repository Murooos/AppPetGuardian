...
def ver_usuario():
    db = DBHandler()
    try:
        query = "SELECT * FROM usuario WHERE ativo = TRUE"
        lista = db.execute(query)
        return lista
    finally:
        db.close()
...
def ver_convenio():
    db = DBHandler()
    try:
        query = "SELECT * FROM convenio WHERE ativo = TRUE"
        lista = db.execute(query)
...
def ver_clinica():
    db = DBHandler()
    try:
        query = "SELECT * FROM clinica WHERE ativo = TRUE"
        lista = db.execute(query)
        return lista
    finally:
        db.close()
...
def ver_veterinario():
    db = DBHandler()
    try:
        query = "SELECT * FROM veterinario WHERE ativo = TRUE"
        lista = db.execute(query)
        return lista
    finally:
        db.close()
...
def ver_historicomedico():
    db = DBHandler()
    try:
        query = "SELECT * FROM historicomedico WHERE ativo = TRUE"
        lista = db.execute(query)
        return lista
    finally:
        db.close()
...
def inserir_usuario(nome, endereco=None, telefone=None, email=None, convenioid=None, senha=None):
    ...
    try:
        ...
        if endereco is not None:
            ...
            valores.append('%(endereco)s')
            params['endereco'] = endereco
        if telefone is not None:
            campos.append('telefone')
            valores.append('%(telefone)s')
            params['telefone'] = telefone
        if email is not None:
            campos.append('email')
            valores.append('%(email)s')
            params['email'] = email
        if convenioid is not None:
            campos.append('convenioid')
            valores.append('%(convenioid)s')
...
def inserir_historicomedico(dataconsulta, animalid, veterinarioid, diagnostico=None, tratamento=None, observacoes=None, exameclinico=None):
    ...
    try:
        ...
        valores = ['%(dataconsulta)s', '%(animalid)s', '%(veterinarioid)s']
        params = {
